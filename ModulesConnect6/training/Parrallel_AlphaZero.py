...
file_handler = logging.FileHandler('./logs/Par_AlphaZero_debug.log', mode='w')
...
folder_path = './ModulesConnect6/checkpoints/Par_alphazero/'
...
EVAL_INTERVAL = 100      # Evaluate every 100 self-play games

# ----------------------------
# (略) 以下為 Connect6、augment_state_policy、Connect6Net、MCTS、rule_based_agent_move、evaluate_agent、self_play_game、train 等函數與類別定義
# 這部分維持原程式碼不變
# ----------------------------
# ----------------------------
...
# 以下為平行化自對弈的輔助函數
# -------------------------------
# 由於多進程間無法直接共享 GPU 模型，這裡示範時將自對弈所用模型移至 CPU，
# 並用全域變數讓子進程使用，實際應用可考慮每個子進程建立一個模型副本。
global_net = None
def init_worker(net_state_dict):
    global global_net, DEVICE
    DEVICE = torch.device('cpu')  # 強制子進程使用 CPU
    net = Connect6Net(history_length=HISTORY_LENGTH)
    net.load_state_dict(net_state_dict)
    net.to(DEVICE)
    net.eval()
    global_net = net

def parallel_self_play(_):
    # 子進程中直接使用全域 global_net 進行自對弈
    return self_play_game(global_net)

# -------------------------------
# 修改後的 Main Training Loop (平行化自對弈生成)
# -------------------------------
def main_training_loop(num_iterations=1000, games_per_iteration=10):
    ...
    for iteration in range(num_iterations):
        ...
        # 每次開始自對弈前，更新模型權重並建立新的 Pool
        # 將 net 轉到 CPU 並取得 state_dict 傳入各子進程
        net_cpu_state = net.cpu().state_dict()
        with Pool(processes=num_processes, initializer=init_worker, initargs=(net_cpu_state,)) as pool:
            # 並行生成 self_play_game
            results = pool.map(parallel_self_play, range(games_per_iteration))
        
        # 將各遊戲結果合併
        for states, mcts_policies, outcomes in results:
            replay_buffer_states.extend(states)
            replay_buffer_policies.extend(mcts_policies)
            replay_buffer_values.extend(outcomes)
            total_games_played += 1
            logger.info(f"Self-play game completed. Total games played: {total_games_played}")
