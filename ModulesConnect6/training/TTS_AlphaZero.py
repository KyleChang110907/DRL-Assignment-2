...
# ----- 新增部分：威脅空間搜尋參數與輔助函數 -----
THREAT_THRESHOLD = 2  # 只有當連線數大於或等於此值時才認為此走法具有威脅性

def compute_max_chain(board, i, j, player):
    """
    計算在棋盤 board 中，放下 player 在 (i,j) 位置後，在水平、垂直及兩個斜線方向上，
    能連續形成的最大連線數。
    """
...
class Connect6:
    ...
    # ----- 新增部分：威脅評估與候選走法過濾 -----
    def evaluate_threat(self, move):
        """
        模擬放置 move 後，計算能連成的最大連線數，作為威脅評估值。
        若為第一手則 move 為一個 tuple；否則為一對 tuple。
        """
        ...
        if self.move_count == 0:
            ...
            return compute_max_chain(board_copy, i, j, player)
        else:
            scores = []
            for pos in move:
                i, j = pos
                board_temp = board_copy.copy()
                board_temp[i, j] = player
                scores.append(compute_max_chain(board_temp, i, j, player))
            return max(scores)

    def get_threat_moves(self):
        """
        從合法走法中篩選出威脅性較強的走法：
        僅保留經 evaluate_threat 評估後威脅值大於等於 THREAT_THRESHOLD 的走法
        """
        legal = self.get_legal_moves()
        threat_moves = []
        for move in legal:
            score = self.evaluate_threat(move)
    ...
    def get_state_tensor(self):
        ...
        對於每個棋盤歷史，產生兩個通道：
         - 當前玩家的棋子。
         - 對手的棋子。
        返回 tensor 形狀為 (2 * history_length, BOARD_SIZE, BOARD_SIZE)。
        """
...
class MCTS:
    ...
    def run(self, game_state):
        ...
        if game_state.move_count == 0:
            ...
            # 嘗試先取得威脅走法（第一階段）
            threat_moves = game_state.get_threat_moves()
            if threat_moves:
                legal_moves = threat_moves
                logger.debug("Using threat moves as candidate moves.")
            else:
                legal_moves = game_state.get_legal_moves()
                logger.debug("No threat moves found, using full legal moves.")
        # -----------------------------------------------
