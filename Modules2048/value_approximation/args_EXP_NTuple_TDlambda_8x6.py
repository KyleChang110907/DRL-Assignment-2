...
if __name__ == "__main__":
    # 解析命令列參數
    parser = argparse.ArgumentParser(description="Train 2048 agent with TD(λ) and N-Tuple Approximator")
    parser.add_argument("--optimistic_init", type=float, default=25, help="樂觀初始化的值")
    parser.add_argument("--alpha", type=float, default=0.1, help="學習率")
    parser.add_argument("--num_episodes", type=int, default=100000, help="訓練的局數")
    parser.add_argument("--folder_path", type=str, default="./Modules2048/checkpoints/value_approximation/Exp_N_tuple_TDlambda_6Tuples_8pattern_lr10-1_OI25/", help="存放檔案的資料夾路徑")
    ...
    # 使用離線版 TD(λ) 進行訓練
    final_scores = td_lambda_offline_learning(env, approximator, num_episodes=num_episodes, alpha=alpha, gamma=0.99, lmbda=0.8)
